...
use Xpmock\TestCase;
use Dhii\Parser\Tokenizer\Exception\CouldNotTokenizeException as TestSubject;
 ...
 * @since [*next-version*]
 */
class CouldNotTokenizeExceptionTest extends TestCase
{
    /**
     ...
     * @since [*next-version*]
     */
    const TEST_SUBJECT_CLASSNAME = 'Dhii\Parser\Tokenizer\Exception\CouldNotTokenizeException';
    ...
    public function testCanBeCreated()
    {
        ...
        $this->assertInstanceOf(
            'Dhii\Parser\Tokenizer\Exception\CouldNotTokenizeExceptionInterface',
            $subject,
    ...
    public function testConstructor()
    {
        ...
        $subject->expects($this->once())
                ->method('_setLineNumber')
                ->with($this->equalTo($lineNumber));
        $subject->expects($this->once())
                ->method('_setColumnNumber')
                ->with($this->equalTo($columnNumber));
        $subject->expects($this->once())
                ->method('_construct');

        $subject->__construct($message, $code, $previous, $tokenizer, $lineNumber, $columnNumber);
